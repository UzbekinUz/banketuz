...
const API_URL = 'http://localhost:5000/api'; // Serveringiz manzili
...
const [activeTab, setActiveTab] = useState('order');
  ...
  const isFormValid = formData.cName.trim().length > 0 && formData.cPhone.trim().length > 0;
  ...
  const handleSvcQtyChange = (svcId, qty) => {
    ...
    setSelectedServices((prev) => ({
      ...prev,
      [svcId]: { ...prev[svcId], qty: parsedQty, self: parsedQty > 0 ? false : prev[svcId]?.self }
    }));
  };
  ...
  const handleSvcSelfChange = (svcId, checked) => {
    setSelectedServices((prev) => ({
      ...prev,
      [svcId]: { ...prev[svcId], self: checked, qty: checked ? 0 : prev[svcId]?.qty }
    }));
  };
  ...
  const handleClearOrder = () => {
    if (!window.confirm("Hamma kiritilgan ma'lumotlarni tozalashni xohlaysizmi?")) return;
    setFormData({
      cName: '', cPhone: '', cReason: '', cDate: '', cTime: '', cGuests: '',
      cOrg: '', cWishes: '', cPayDate: '', cPayMethod: 'Наличные', cPayOther: '', cNotes: ''
    });
    setSelectedSubCats({});
    setSelectedServices({});
    setDiscountPct(0);
  ...
  const saveSettings = async () => {
    try {
      ...
      const url = isEdit ? `${API_URL}/settings/edit/${settings._id}` : `${API_URL}/settings`;
  ...
  const addService = async () => {
    ...
    if (!title) return;
    try {
      const res = await axios.post(`${API_URL}/defServices/add`, { title, price: "0" });
      if (res.status === 200 || res.status === 201) {
  ...
  const addCategory = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/categories/add`, { smile, title });
      if (res.status === 200 || res.status === 201) {
  ...
  const deleteCategory = async (id) => {
    if (!window.confirm("Ushbu bo'lim va uning barcha taomlari o'chiriladi. Rozimisiz?")) return;
    try {
  ...
  const updateSubCategoryItem = async (subId, fields) => {
    try {
      await axios.put(`${API_URL}/sub-categories/edit/${subId}`, fields);
  ...
  const downloadPdf = async () => {
    ...
    // Order buyurtmasini saqlash API ga yuboriladi
    const orderData = {
      name: formData.cName,
      tel: formData.cPhone,
      ...
      subCategory: Object.keys(selectedSubCats).filter(id => selectedSubCats[id] > 0),
      additionalServices: services.map(svc => ({
        title: svc.title,
        price: svc.price,
      ...
      prepayment: {
        ...
        typePayment: formData.cPayMethod === 'Другое' ? formData.cPayOther : formData.cPayMethod,
    ...
    try {
      ...
      pdf.save(`Order_${formData.cName.replace(/\s+/g, '_')}.pdf`);
    } catch (err) {
    ...
    <div className="wrap">
      ...
      <div className="field-grid">
        <div className="field">
          ...
          <input type="text" id="cName" value={formData.cName} onChange={handleInputChange} placeholder="Ф.И.О." />
        </div>
        <div className="field">
          <label>Телефон</label>
          <input type="text" id="cPhone" value={formData.cPhone} onChange={handleInputChange} placeholder="+998 90 000 00 00" />
        </div>
        <div className="field">
          <label>Повод мероприятия</label>
          <input type="text" id="cReason" value={formData.cReason} onChange={handleInputChange} placeholder="Свадьба, юбилей, корпоратив..." />
        </div>
        <div className="field">
          <label>Дата мероприятия</label>
          <input type="date" id="cDate" value={formData.cDate} onChange={handleInputChange} />
        </div>
        <div className="field">
          <label>Время начала</label>
          <input type="time" id="cTime" value={formData.cTime} onChange={handleInputChange} />
        </div>
        <div className="field">
          <label>Количество гостей (чел.)</label>
          <input type="number" id="cGuests" min="1" value={formData.cGuests} onChange={handleInputChange} placeholder="например, 50" />
        </div>
        <div className="field full">
          <label>Экспертиза органи / Название организации</label>
          <input type="text" id="cOrg" value={formData.cOrg} onChange={handleInputChange} placeholder="При наличии" />
        </div>
        <div className="field full">
          <label>Доп. информация / пожелания</label>
          <textarea id="cWishes" value={formData.cWishes} onChange={handleInputChange} placeholder="Особые пожелания, аллергии и т.д."></textarea>
        </div>
      </div>
      ...
      <div>
        {categories.map((cat) => {
          const catSubItems = subCategories.filter((sub) => sub.category === cat._id);
          const selectedCountInCat = catSubItems.reduce((acc, item) => acc + (selectedSubCats[item._id] ? 1 : 0), 0);
          const isOpen = !!openCategories[cat._id];

          return (
            <div key={cat._id} className={`category ${isOpen ? 'open' : ''}`}>
              <div className="category-head" onClick={() => toggleCategory(cat._id)}>
                <div className="title">
                  <span className="icon">{cat.smile || '🍽️'}</span>
                  <h3>{cat.title}</h3>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span className={`count-badge ${selectedCountInCat > 0 ? 'show' : ''}`}>
                    ...
                    return (
                      <div key={sub._id} className="item-row">
                        <div className="item-name">
                          ...
                          {sub.title} {sub.size && <span style={{ fontSize: '12px', color: 'var(--ink-soft)' }}>({sub.size})</span>}
                          {!sub.bor && <span className="neg-tag">Mavjud emas</span>}
                        </div>
                        <div className="item-price">{price.toLocaleString()} сум</div>
                        <div>
                          <input
                            type="number"
                            className="qty-input"
                            ...
                            value={qty || ''}
                            disabled={!sub.bor}
                            onChange={(e) => handleSubQtyChange(sub._id, e.target.value)}
                            placeholder="0"
                          />
                        </div>
                        <div className="line-total">{lineTotal.toLocaleString()} сум</div>
                      </div>
                    );
                  })}
                </div>
      ...
      <div className="services-wrap">
        <table className="services-table">
          <thead>
            <tr>
              <th rowSpan="2" className="svc-name-col">Наименование услуги</th>
              <th colSpan="3">Если организуем мы</th>
              <th rowSpan="2" className="svc-self-col">Клиент<br />сам</th>
            </tr>
            <tr>
          ...
          <tbody>
            {services.map((svc) => {
              const svcState = selectedServices[svc._id] || { qty: 0, self: false };
                ...
                <tr key={svc._id}>
                  ...
                  <td>
                    <input
                      ...
                      value={svcState.qty || ''}
                      onChange={(e) => handleSvcQtyChange(svc._id, e.target.value)}
                      placeholder="0"
                    />
                  </td>
                  <td className="svc-price">{price.toLocaleString()} сум</td>
                  ...
                  <td>
                    <input
                      ...
                      onChange={(e) => handleSvcSelfChange(svc._id, e.target.checked)}
                    />
                  </td>
                </tr>
              );
      ...
      <div className="field-grid">
        <div className="field">
          ...
          <input type="number" id="prepayPct" min="0" max="100" value={prepayPct} onChange={(e) => setPrepayPct(e.target.value)} placeholder="0" />
        </div>
        <div className="field">
        ...
        <div className="field">
          ...
          <input type="date" id="cPayDate" value={formData.cPayDate} onChange={handleInputChange} />
        </div>
        <div className="field">
          <label>Способ оплаты</label>
          <select id="cPayMethod" value={formData.cPayMethod} onChange={handleInputChange}>
            <option value="Наличные">Наличные</option>
            <option value="Пластиковая карта">Пластиковая карта</option>
            <option value="Банковский перевод">Банковский перевод</option>
            <option value="Смешанный (наличные + карта)">Смешанный (наличные + карта)</option>
        ...
        {formData.cPayMethod === 'Другое' && (
          <div className="field full">
            ...
            <input type="text" id="cPayOther" value={formData.cPayOther} onChange={handleInputChange} placeholder="Уточните способ оплаты" />
          </div>
        )}
        <div className="field full">
      ...
      <div className="field-grid">
        <div className="field full">
          <textarea id="cNotes" value={formData.cNotes} onChange={handleInputChange} placeholder="Дополнительные примечания менеджера или заказчика"></textarea>
        </div>
      </div>
      ...
      <div className="totals-box">
        ...
        <div className="totals-row">
          <span className="lbl">
            Скидка, %
            <input type="number" className="mini-input" min="0" max="100" value={discountPct} onChange={(e) => setDiscountPct(e.target.value)} />
          </span>
        ...
        <div className="totals-row">
          <span className="lbl">
            Обслуживание, %
            <input type="number" className="mini-input" min="0" max="100" value={servicePct} onChange={(e) => setServicePct(e.target.value)} />
          </span>
      ...
      <div className="doc-footer">
        ...
        <div className="final-leaf"><span className="leaf">&#10087;</span></div>
      </div>
    </div>
  )}
      ...
      {!isAdminUnlocked ? (
        <div className="admin-gate">
          ...
          <input
            ...
            maxLength="12"
          />
          {pinErr && <div className="err" style={{ display: 'block' }}>Kod noto'g'ri, qaytadan urinib ko'ring</div>}
          <button className="btn btn-gold" style={{ width: '100%' }} onClick={checkPin}>Kirish</button>
        </div>
      ) : (
        <div>
          <div className="admin-toolbar">
            <div>
              ...
              <div className="sub">O'zgarishlar saqlangach barcha mijozlarga darhol ko'rinadi</div>
            </div>
            <button className="btn btn-ghost" style={{ color: '#6d5620', borderColor: '#6d5620' }} onClick={() => setIsAdminUnlocked(false)}>
              Chiqish
            </button>
          </div>
          ...
          <div className="admin-settings">
            ...
            <div className="grid3">
              ...
              <input
                ...
                onChange={(e) => setSettings({ ...settings, insta: e.target.value })}
                onBlur={saveSettings}
          ...
          <div className="admin-cat">
            <div className="admin-cat-head">
              <input type="text" className="name-input" value="Qo'shimcha xizmatlar narxlari" disabled style={{ opacity: 0.8 }} />
            </div>
            <div>
              {services.map((svc) => (
                <div key={svc._id} className="admin-service-row">
                  <input
                    ...
                    onBlur={(e) => updateService(svc._id, e.target.value, svc.price)}
                  />
                  <input
                    type="number"
                    defaultValue={svc.price}
                    onBlur={(e) => updateService(svc._id, svc.title, e.target.value)}
                  />
                  <div />
                  <button className="icon-btn danger" onClick={() => deleteService(svc._id)}>✕</button>
                </div>
              ))}
            </div>
            <div className="admin-add-row">
              <button className="btn btn-gold" onClick={addService}>+ Xizmat qo'shish</button>
            </div>
          </div>

          {/* Admin Categories & SubCategories Block */}
          <div className="admin-new-cat">
            <input type="text" className="icon-new" id="newCatIcon" placeholder="🍽️" maxLength="4" />
            <input type="text" id="newCatName" placeholder="Yangi bo'lim nomi (masalan, ICHIMLIKLAR)" />
            <button className="btn btn-gold" onClick={addCategory}>+ Bo'lim qo'shish</button>
          </div>
                ...
                <div key={cat._id} className="admin-cat">
                  <div className="admin-cat-head">
                    <input
                      ...
                      onBlur={(e) => updateCategory(cat._id, e.target.value, cat.title)}
                    />
                    <input
                      type="text"
                      className="name-input"
                      defaultValue={cat.title}
                      onBlur={(e) => updateCategory(cat._id, cat.smile, e.target.value)}
                    />
                    <button className="icon-btn danger" onClick={() => deleteCategory(cat._id)}>Bo'limni o'chirish</button>
                  </div>
                    ...
                    {catSubItems.map((sub) => (
                      <div key={sub._id} className="admin-item-row">
                        <input
                          ...
                          onBlur={(e) => updateSubCategoryItem(sub._id, { title: e.target.value })}
                        />
                        <input
                          type="text"
                          defaultValue={sub.size || ''}
                          placeholder="Hajmi"
                          onBlur={(e) => updateSubCategoryItem(sub._id, { size: e.target.value })}
                        />
                        <input
                          type="number"
                          defaultValue={sub.price}
                          onBlur={(e) => updateSubCategoryItem(sub._id, { price: e.target.value })}
                        />
                        <button className="icon-btn danger" onClick={() => deleteSubCategoryItem(sub._id)}>✕</button>
                      </div>
                    ))}
                  </div>

                  <div className="admin-add-row">
                    <button className="btn btn-gold" onClick={() => addSubCategoryItem(cat._id)}>+ Taom (SubCategory) qo'shish</button>
                  </div>
                </div>
              );
            })}
    ...
    <div className="summary-bar">
      <div className="summary-inner">
        <div className="summary-total">
          <div className="label"><span>{calcSelectedItemCount()}</span> ta taom &middot; ИТОГО К ОПЛАТЕ</div>
          <div className="amount">{grandTotal.toLocaleString()} сум</div>
        </div>
        <div className="summary-actions">
          <button className="btn btn-ghost" onClick={handleClearOrder}>Tozalash</button>
          <button
  ...
  <div id="printableOrder">
    ...
    <div className="p-contact">
      Тел: {settings.phone || '—'} &nbsp;|&nbsp; Instagram: {settings.insta || '—'} &nbsp;|&nbsp; Манзил: {settings.addr || '—'}
    </div>

    <div className="p-band">1. ОСНОВНЫЕ ДАННЫЕ</div>
    <div className="p-fields">
      <div><span className="k">Имя заказчика: </span><span className="v">{formData.cName}</span></div>
      <div><span className="k">Телефон: </span><span className="v">{formData.cPhone}</span></div>
      <div><span className="k">Повод: </span><span className="v">{formData.cReason}</span></div>
      <div><span className="k">Дата / Время: </span><span className="v">{formData.cDate} / {formData.cTime}</span></div>
      <div><span className="k">Количество гостей: </span><span className="v">{formData.cGuests} чел.</span></div>
      <div><span className="k">Организация: </span><span className="v">{formData.cOrg}</span></div>
      <div className="full"><span className="k">Пожелания: </span><span className="v">{formData.cWishes}</span></div>
    </div>
    ...
    <table>
      ...
      <tbody>
        {categories.map(cat => {
          const catSubs = subCategories.filter(s => s.category === cat._id && (selectedSubCats[s._id] || 0) > 0);
          if (catSubs.length === 0) return null;
          return (
            <React.Fragment key={cat._id}>
              <tr className="p-cat-row">
                <td colSpan="4">{cat.smile} {cat.title}</td>
              </tr>
              {catSubs.map(sub => {
                ...
                return (
                  <tr key={sub._id}>
                    <td>{sub.title} {sub.size ? `(${sub.size})` : ''}</td>
                    <td className="num">{qty}</td>
                    <td className="num">{price.toLocaleString()}</td>
                    <td className="num">{(qty * price).toLocaleString()}</td>
                  </tr>
                );
              })}
            </React.Fragment>
        ...
        {services.map(svc => {
          ...
          return (
            <tr key={svc._id}>
              ...
              <td className="num">{st.self ? '-' : price.toLocaleString()}</td>
              <td className="num">{st.self ? '0' : (st.qty * price).toLocaleString()}</td>
            </tr>
          );
        })}
      </tbody>
    ...
    <div className="p-totals">
      <div className="p-trow"><span>Итого по меню:</span><span className="v">{subtotal.toLocaleString()} сум</span></div>
      <div className="p-trow"><span>Скидка ({discountPct}%):</span><span className="v">{discountSum.toLocaleString()} сум</span></div>
      <div className="p-trow"><span>Итого по 1 чел:</span><span className="v">{perPerson.toLocaleString()} сум</span></div>
      <div className="p-trow"><span>Обслуживание ({servicePct}%):</span><span className="v">{serviceSum.toLocaleString()} сум</span></div>
      <div className="p-trow"><span>Предоплата ({prepayPct}%):</span><span className="v">{prepaySum.toLocaleString()} сум</span></div>
      <div className="p-final"><span>ИТОГО К ОПЛАТЕ:</span><span>{grandTotal.toLocaleString()} сум</span></div>
    </div>
